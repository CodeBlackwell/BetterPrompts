...
/**
 * Test Data Setup/Teardown Script for Phase 10 Rate Limiting Tests (TypeScript version)
 * 
 * This script directly manages test API keys in the database when admin API endpoints
 * are not available or returning errors.
 * 
 * Usage:
 *   npx ts-node test-data-setup.ts setup    # Create test API keys
 *   npx ts-node test-data-setup.ts teardown # Remove test API keys
 *   npx ts-node test-data-setup.ts reset    # Reset rate limit counters
 */
...
async function setupDatabase(): Promise<void> {
  ...
  try {
    ...
    // Create test users
    const userIds = [...new Set(testApiKeys.map(k => k.user_id))];
...
async function resetRateLimits(): Promise<void> {
  ...
  try {
    ...
    const keys = await keysAsync('rate_limit:test-key-*');
...
// Integration with Playwright global setup/teardown
export async function globalSetup(): Promise<void> {
  console.log('Running Phase 10 test data setup...');
  await setupDatabase();
  await resetRateLimits();
}

export async function globalTeardown(): Promise<void> {
  console.log('Running Phase 10 test data teardown...');
  
  // Only teardown if explicitly requested
...
async function main(): Promise<void> {
  ...
  try {
    switch (command) {
      ...
      default:
        console.log('Usage:');
        console.log('  npx ts-node test-data-setup.ts setup    # Create test API keys');
        console.log('  npx ts-node test-data-setup.ts teardown # Remove test API keys');
        console.log('  npx ts-node test-data-setup.ts reset    # Reset rate limit counters');
