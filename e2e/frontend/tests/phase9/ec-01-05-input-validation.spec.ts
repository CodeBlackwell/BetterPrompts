...
const OUTPUT_SELECTOR = '.enhanced-prompt, .output, [data-testid="enhanced-prompt"], [data-testid="anonymous-enhanced-output"]';
const ERROR_SELECTOR = '.error-message, .alert-error, [role="alert"], [data-testid="error-message"]';
const CHAR_COUNT_SELECTOR = '[data-testid="anonymous-character-count"], .text-sm.text-muted-foreground';
  ...
  test.describe('EC-01: Character Limit Enforcement', () => {
    ...
    for (const testCase of charLimitCases) {
      test(`should handle ${testCase.name}`, async ({ page }) => {
        ...
        await expect(page.locator(SUBMIT_BUTTON_SELECTOR)).toBeEnabled({ timeout: INPUT_TIMEOUT });
